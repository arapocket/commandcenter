...
module.exports.getAllGuards = function (callback) {
    ...
    db.createConnection(function (err, reslt) {
        if (err) {
            ...
            var strSQL = ' SELECT FirstName, LastName, DeviceToken, GuardID, CurrentPatrol, CurrentCoord FROM currentguards WHERE CurrentPatrol = 1; ';
...
module.exports.getCurrentLocations = function (callback) {
    ...
    db.createConnection(function (err, reslt) {
        if (err) {
            ...
            var strSQL = 'SELECT lat, lng, GuardID, FirstName, LastName, DeviceToken FROM currentguards WHERE CurrentCoord = 1 GROUP BY GuardID';
            connection.query(strSQL, function (err, rows, fields) {
                if (!err) {
                    connection.end();
                    callback(null, rows);
