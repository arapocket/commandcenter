...
module.exports.updateCoordinate = function (Coordinate, callback) {
    ...
    db.createConnection(function (err, reslt) {
        if (err) {
            ...
            var strSQL = "Update coordinate SET CurrentCoord = " + Coordinate.CurrentCord + " WHERE CoordID =  '" + Coordinate.CoordID + "';"; connection.query(strSQL, function (err, rows, fields) {
                if (!err) {
                    connection.end();
                    callback(null, rows);
